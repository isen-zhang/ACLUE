...
from categories import zh2en,taskno,category,task2cate
choices = ["A", "B", "C", "D"]
...
def run_eval(model, tokenizer, eval, args):
    ...
    for subject in subjects:
        ...
        if os.path.exists(out_file):  # If result file exist, skip this subject
            continue
        dev_df = pd.read_csv(os.path.join(args.data_dir, "dev", subject + ".csv"), header=0, index_col=None)
        test_df = pd.read_csv(os.path.join(args.data_dir, "test", subject + ".csv"), header=0, index_col=None)

        acc, preds = eval(model=model,
                                 tokenizer=tokenizer,
                                 subject=subject,
                                 dev_df=dev_df,
                                 test_df=test_df,
    ...
    # print result
    get_results(subjects,args.save_dir)
...
def get_results(subjects, result_dir=''):
    ...
    for subject in subjects:
        ...
        if num_columns == 8:
            ...
            df = pd.read_csv(file, names=['question', 'A', 'B', 'C', 'D', 'answer', 'response'])

        #df = pd.read_csv(, names=['id','question','A','B','C','D','answer','response'], index_col=0)
        ...
        all_acc[subject]=acc
    subjects = sorted(taskno,key=taskno.get)
    print_content = ''
    category_dict = defaultdict(list)
    for subject in subjects:
        category_dict[task2cate[subject]].append(all_acc[subject])
        #print_content +=f"|   {all_acc[subject]:.1f}   "
        #print(f"{subject} {taskno[subject]} {all_acc[subject]:.2f}")
    for c in category:
        print_content +=f"|   {np.mean(category_dict[c]):.2f}   "
    avg_all_acc = np.mean(list(all_acc.values()))
    print_content += f"|   {avg_all_acc:.2f}   |"
    print(print_content)
